...
We know for sure that the cell on the right can belong either to the current processor or the previous one, and the one below similarly either to the current or to the next one, but we couldn't find an usage of this information to improve the algorithm speed.
...
```
<!--stackedit_data:
eyJoaXN0b3J5IjpbMzU1NjkyODgsLTgwNzIwNTU1NSw4Njg5NT

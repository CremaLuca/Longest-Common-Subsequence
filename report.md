...
Each entry in $L(d)$ will depend only on entries belonging to $L(d-1)$ and L($d-2)$. In fact each element depends only on three elements from the two previous principal diagonals. This suggests a way to parallelize our initial algorithm: each entry in the same diagonal 
...
```
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTI1MjcyOTcxNyw1Mzk5MTcxNjQsNzA3NT

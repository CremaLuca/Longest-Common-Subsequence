...
**Remark:** It's quite easy to see that $P(i, j)$, during the computation of entry $(i, j)$,  already has the value of cell $(i-1, j-1)$ stored in its memory. First notice that at least one of $P(i, j-1) = p$ or $(i-1, j)$ is assigned to $p$:  in fact, let's say cell $(i, j)$ lies on diagonal $d$; then if diagonal $d-1$ has the same length as diagonal $d$, it follows that processor
...
```
<!--stackedit_data:
eyJoaXN0b3J5IjpbMTAzMTQwNTI0OCwtMTA5MzgzMzAxLC0xMD

...
Given the coordinates $(i, j)$ the diagonal is clearly  $i+j$, while $\text{pos(i, j)}$ is given by row $i$ if  $d < n$ and $N-j-1$ otherwise.
...
```
<!--stackedit_data:
eyJoaXN0b3J5IjpbNTQ3MjcwMDcyLDUzODcyMzQ0MSw1ODc2MT

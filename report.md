...
Each entry in $L(d)$ will depend only on entries belonging to $L(d-1)$ and L($d-2)$. In fact each element depends only on three elements from the two previous principal diagonals.
...
```
<!--stackedit_data:
eyJoaXN0b3J5IjpbMjA3NjUwMzU1OSw1Mzk5MTcxNjQsNzA3NT

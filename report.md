...
Foreach principal diagonal $d$ we know how many elements belong to each processor using the values from `diag_start_end` $s$ and $e$. We also know that we might need $s-1$ and $e+1$ to compute the cells of the next principal diagonal $d+1$. So we can store 
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTM1MDUyNTE2LDE4MTc4NzQxMzQsMTY0NT

...
It's easy to derive a formula from the previous assignment of processors to each diagonal; we have:$$P(i, j)=\begin{cases}a& \text
{if $0\le d < m$}  \\b & \text{if $m \le d < n$} \\\{M[d-n + 1, n-1], M[N-d+2, n-2], \ldots,  M[m-1, d-m+1])\}  & \text{if $d \ge n$}\end{cases}$$
...
```
<!--stackedit_data:
eyJoaXN0b3J5IjpbMjgzNDg4Njg1LDUzODcyMzQ0MSw1ODc2MT

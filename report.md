...
A performance metric we use for the assignment is the number of messages exchanged by the processors. The exact measure for variable $n, m$ and $p$ is hard to obtain from analytical considerations, but we can give an upper bound: clearly we can assume $P_{\text{max}} = \min\{n, m\} = m$
...
```
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTc2NDA1OTA0MywtMTM5MTIwNTcwLDE2OD

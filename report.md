...
Each entry in $L(d)$ will depend only on entries belonging to $L(d-1)$ and L($d-2)$. In fact each element depends only on three elements from the two previous principal diagonals. This suggests a way to parallelize our initial algorithm: by looking at the CDAG of the computation, each diagonal is a level of the greedy schedule. Hence
...
```
<!--stackedit_data:
eyJoaXN0b3J5IjpbMjA3MTk1NTk4NCw1Mzk5MTcxNjQsNzA3NT
